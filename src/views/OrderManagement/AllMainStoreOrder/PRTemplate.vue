<template>
  <div class="office-block">
    ...
    <span class="templateDel" @click="delProduct(product.PRTemplateId)">
    ...
    <div class="store-list" v-for="(PRItem, key) of detailRows" :key="key">
      <div class="store bg-white">
        <span class="pendingOrder" v-show="PRItem.IsPending"> 掛單 </span>
        ...
        <VSelect
          ...
          valueFormat="object"
          :options="filterStore"
          v-model="PRItem.store"
          :disabled="PRItem.IsPending && PRItem.store && product.InvQty > 0"
          @input="changeStore(PRItem)"
        >
        ...
        <input
          ...
          type="number"
          placeholder="請輸入數量"
          :disabled="!PRItem.store || (PRItem.IsPending && product.InvQty > 0)"
          @keyup="checkQuantity(PRItem, '訂購')"
          @change="checkQuantity(PRItem, '訂購')"
          v-model.number="PRItem.orderQuantity"
        />
        <!-- 是掛單就不能有寄倉 -->
        <input
          class="amount col-2"
          type="number"
          placeholder="請輸入數量"
          :disabled="!PRItem.store || PRItem.IsPending"
          @keyup="checkQuantity(PRItem, '寄倉')"
          @change="checkQuantity(PRItem, '寄倉')"
          v-model.number="PRItem.warehousesNum"
        />
...
export default {
  ...
  methods: {
    ...
    delItem(key, item) {
      ...
      vm.$nextTick(() => {
        ...
        if (vm.detailRows.length === 0) {
          setTimeout(() => {
            vm.$emit("removeTemplate", vm.product.PRTemplateId, "prRows", true);
          }, 50);
        }
      });
    },
    ...
    delProduct(key) {
      ...
      vm.$nextTick(() => {
        setTimeout(() => {
          vm.$emit("removeTemplate", key, "prRows", true);
        }, 50);
      });
    },

    // 檢查數量，假如大於庫存就顯示 Modal
    checkQuantity(PRItem, type) {
      const vm = this;

      if (type === "寄倉") {
        if (PRItem.warehousesNum > PRItem.orderQuantity) {
          vm.$notify({
            title: "提示",
          ...
          vm.$nextTick(() => {
            PRItem.warehousesNum = PRItem.orderQuantity;
          });
          return;
        }
      }
      vm.currentValue = Number(PRItem.orderQuantity);
      // PRItem 沒有 product 資料，所以要添加
      PRItem.productData = [];
      PRItem.productData[0] = {
        IsPartialDelivery: false, // 是否分批
        PartialDeliveryQuantity: 0, // 分批出貨數量
        IsPending: PRItem.IsPending,
        orderQuantity: PRItem.orderQuantity, // 訂購數量
        warehousesNum: PRItem.warehousesNum, // 寄倉數量
        StoreGuid: PRItem.Guid, // 這間分店的 Guid
        ...vm.exceptProduct,
      };

      // 改變訂購數量時順便改變確認訂單內的數量。
      vm.$store.commit("CHANGEBRANCHQUANTITY", PRItem);
      ...
      if (vm.nowTotal > vm.product.InvQty && vm.totalMinusNow < 0) {
        vm.inputType = type;
        vm.prGuid = PRItem.Guid;
        vm.prStore = PRItem.store;

        vm.$nextTick(() => {
          // console.log(vm.$refs[PRItem.Guid]);
          let element = vm.$refs[PRItem.Guid].$el;
          $(element).modal("show");
        });
      }
    },
    ...
    changeStore(PRItem) {
      ...
      // this.print("改變分店", PRItem);
      branchStoreData = {
        TaxId: PRItem.store.TaxID, // 假如分店分別寄送，就要帶入各家店自己的統編
        ReceiveName: PRItem.store.Owner, // 預設帶出收貨人名稱
        ReceiveContact: PRItem.store.TEL_NO, // 預設帶出收貨人電話
        ReceiveAddress: PRItem.store.ShippingAddress, // 預設帶出收貨人電話
        InvoiceName: PRItem?.store?.Contact || "", // 預設帶出發票收件人名稱
        InvoiceContact: PRItem?.store?.TEL_NO || "", // 預設帶出發票收件人電話
        InvoiceAddress: PRItem?.store?.InvoiceAddress || "", // 預設帶出發票收件人地址
        Guid: PRItem.Guid, // 這間分店的 Guid
        productData: [...PRItem.productData], // 該間分店目前商品的資訊
        store: { ...PRItem.store }, // 該間分店自己的資訊
      };
    ...
    InventoryShortItem(val, type, orderOrWare) {
      ...
      if (type) {
        ...
        vm.detailRows.some((order) => {
          if (order.Guid === val.Guid) {
            ...
            order.warehousesNum = 0; // 掛單的話寄倉為 0
            return true;
          }
        });
      }
      ...
      // 從庫存不足 Modal 取得資料後，再送出去保持一致
      // 路徑: InventoryShortageModal > PRTemplate > AllMainStoreOrder
      vm.$emit(
        "addNewDetailRows",
        vm.detailRows,
        "PR",
